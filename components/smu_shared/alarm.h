 ...
 *
 * Time base: uint32_t milliseconds, not int64_t microseconds
 * (FIRMWARE_DESIGN_SPEC.md §4.1). This compiles into the SMU image
 * (Cortex-M23, no FPU, 4 KB RAM) as well as the ESP32's — 64-bit
 * arithmetic on the M23 is multi-instruction and this appears throughout
 * the hot path. Millisecond resolution is ample: the shortest
 * configurable interval anywhere in the system is an on-delay in whole
 * milliseconds.
 *
 * This introduces a wrap at 49.7 days of continuous uptime, on a device
 * meant to run for months. Every comparison against elapsed time MUST use
 * the wrap-safe subtraction idiom:
 *
 *     if ((uint32_t)(now_ms - since_ms) >= delay_ms)   // correct across wrap
 *     if (now_ms >= since_ms + delay_ms)                // WRONG — breaks at wrap
 *
 * because unsigned subtraction wraps consistently with how now_ms itself
 * wrapped, while addition on the right-hand side does not.
 */
#pragma once
...
typedef struct {
    ...
    uint32_t time_ms[TRANSIENT_HISTORY];
    uint8_t  head;
    uint8_t  count;
} transient_hist_t;

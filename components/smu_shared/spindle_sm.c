...
static void enter(spindle_sm_t *sm, spindle_state_t next, uint32_t now_ms)
{
...
void spindle_sm_init(spindle_sm_t *sm, uint32_t now_ms)
{
                       ...
                       uint32_t now_ms)
{
    ...
    const uint32_t in_state_ms = now_ms - sm->state_since_ms;

    switch (sm->state) {

    case SPINDLE_STOPPED:
        if (turning) enter(sm, SPINDLE_SPIN_UP, now_ms);
        break;

    case SPINDLE_SPIN_UP:
        if (!turning) {
            enter(sm, SPINDLE_COAST_DOWN, now_ms);
        } else if (in_state_ms >= (uint32_t)cfg->settle_ms) {
    ...
    case SPINDLE_IDLE:
        if (!turning) {
            ...
            enter(sm, SPINDLE_CUTTING, now_ms);
        }
        break;
    ...
    case SPINDLE_CUTTING:
        if (!turning) {
             ...
             * the stop. */
            sm->last_cycle_ms = now_ms - sm->cut_started_ms;
        ...
        if (!sm->monitoring_armed &&
            in_state_ms >= (uint32_t)cfg->alarm_inhibit_ms) {
    ...
    case SPINDLE_COAST_DOWN:
        if (turning) {
            ...
            enter(sm, SPINDLE_STOPPED, now_ms);
        }
        break;

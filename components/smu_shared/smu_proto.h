...
/* Sensor status — deliberately ordinal-identical to sensor_status_t
 * (scaling.h): SENSOR_OK=0, SENSOR_OPEN=1, SENSOR_OVERRANGE=2,
 * SENSOR_UNDERRANGE=3. That is the complete vocabulary pressure_scale()
 * and current_scale() actually produce, so a direct cast between the two
 * is correct, not just convenient. Earlier drafts of this header
 * additionally had SMU_SENSOR_SHORT and SMU_SENSOR_UNCAL, neither of
 * which corresponds to anything the shared scaling logic actually
 * detects — SHORT was never distinguished from OVERRANGE, and UNCAL
 * duplicated what SMU_DIAG_CAL_DEFAULTED already reports. A wire status
 * nothing ever diagnoses correctly is worse than not having it, so both
 * were removed rather than left as dead codes a receiver could
 * mis-decode. */
#define SMU_SENSOR_OK          0U
#define SMU_SENSOR_OPEN        1U
#define SMU_SENSOR_OVERRANGE   2U
#define SMU_SENSOR_UNDERRANGE  3U
...
/* ============================================================
 * Identity — SMU_REG_IDENT, read-only, 20 bytes
 * ============================================================ */
...
typedef struct __attribute__((packed)) {
    ...
    /* Echo of the nonce from the most recently PROCESSED smu_command_t.
     * Without this, "command not yet seen" and "seen, and this is its
     * result" are indistinguishable from the master's side — a retried
     * command and a stuck result code look identical. The master compares
     * this against the nonce it sent; only once they match does
     * last_cmd_result mean anything about *that* command. */
    uint32_t last_cmd_nonce;
...
_Static_assert(sizeof(smu_ident_t)     == 20,  "ident must be 20 bytes");

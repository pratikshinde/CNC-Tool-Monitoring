 ...
 *
 * Time base: uint32_t milliseconds — see alarm.h's header comment for why
 * (this compiles into the SMU image too) and for the wrap-safe comparison
 * idiom every elapsed-time check here must use.
 */
#pragma once
...
typedef struct {
    ...
    uint32_t        cut_started_ms;   /* valid while CUTTING */

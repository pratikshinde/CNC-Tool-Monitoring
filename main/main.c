 ...
 *   2. dio       — outputs into their safe state as early as possible
 *   3. analog    — fail loudly if the ADC is missing
 *   4. rpm       — needs config for the glitch filter
 *   5. monitor   — starts the real-time loop, which releases the outputs
 *                  after its first complete pass
 *   6. wifi, modbus, web — networking, all on CORE_NETWORK so nothing here
...
void app_main(void)
{
    ...
    /* --- 2. Digital I/O, safe state first ---------------------------- */

    ESP_ERROR_CHECK(dio_init(cfg));
    ...
    if (err != ESP_OK) {
         ...
         * one that comes up, serves its diagnostics page and says exactly
         * what is wrong. Outputs stay safe because the monitor loop will
         * never report healthy. */
        ESP_LOGE(TAG, "analogue front end unavailable: %s",
                 esp_err_to_name(err));
        ESP_LOGE(TAG, "check I2C wiring to the ADS1115 at 0x%02X",
                 ADS1115_I2C_ADDR);
    }

    /* --- 4. RPM inputs ------------------------------------------------ */

    ESP_ERROR_CHECK(rpm_init(cfg));
